...
public class ImpactorUtil {
    ...
    public static boolean deposit(ServerPlayerEntity player, double amount, String currencyKey) {
        return getAccount(player.getUuid(), currencyKey).join().depositAsync(BigDecimal.valueOf(amount)).join().successful();
    }

    public static boolean setBalance(ServerPlayerEntity player, double amount, String currencyKey) {
        return getAccount(player.getUuid(), currencyKey).join().setAsync(BigDecimal.valueOf(amount)).join().successful();
    }
}
